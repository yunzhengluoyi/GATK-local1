...
@Argument(fullName="likelihoodCalculationEngine", shortName="likelihoodEngine", doc="What likelihood calculation engine to use to calculate the relative likelihood of reads vs haplotypes", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="heterogeneousKmerSizeResolution", shortName="hksr", doc="How to solve heterogeneous kmer situations using the fast method", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="sample_name", shortName="sn", doc="Name of single sample to use from a multi-sample bam", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="GVCFGQBands", shortName="GQB", doc="GQ thresholds for reference confidence bands", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="indelSizeToEliminateInRefModel", shortName="ERCIS", doc="The size of an indel to check for in the reference model", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="includeUmappedReads", shortName="unmapped", doc="Include unmapped reads with chromosomal coordinates", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="useAllelesTrigger", shortName="allelesTrigger", doc="Use additional trigger on variants found in an external alleles file", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="keepRG", shortName="keepRG", doc="Only use reads from this read group when making calls (but use all reads to build the assembly)", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
var keepRG: String = _

/** Just determine ActiveRegions, don't perform assembly or calling */
@Argument(fullName="justDetermineActiveRegions", shortName="justDetermineActiveRegions", doc="Just determine ActiveRegions, don't perform assembly or calling", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
var justDetermineActiveRegions: Boolean = _

/** Perform assembly but do not genotype variants */
@Argument(fullName="dontGenotype", shortName="dontGenotype", doc="Perform assembly but do not genotype variants", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
var dontGenotype: Boolean = _

/** Do not analyze soft clipped bases in the reads */
@Argument(fullName="dontUseSoftClippedBases", shortName="dontUseSoftClippedBases", doc="Do not analyze soft clipped bases in the reads", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="captureAssemblyFailureBAM", shortName="captureAssemblyFailureBAM", doc="Write a BAM called assemblyFailure.bam capturing all of the reads that were in the active region when the assembler failed for any reason", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
var captureAssemblyFailureBAM: Boolean = _

/** Use an exploratory algorithm to error correct the kmers used during assembly */
@Argument(fullName="errorCorrectReads", shortName="errorCorrectReads", doc="Use an exploratory algorithm to error correct the kmers used during assembly", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
var errorCorrectReads: Boolean = _

/** The PCR indel model to use */
@Argument(fullName="pcr_indel_model", shortName="pcrModel", doc="The PCR indel model to use", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="maxReadsInRegionPerSample", shortName="maxReadsInRegionPerSample", doc="Maximum reads in an active region", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
...
@Argument(fullName="minReadsPerAlignmentStart", shortName="minReadsPerAlignStart", doc="Minimum number of reads sharing the same alignment start for each genomic location in an active region", required=false, exclusiveOf="", otherArgumentRequired="", validation="")
